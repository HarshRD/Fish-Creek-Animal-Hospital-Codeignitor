...
foreach($query1 as $sql){
	$dropdown1[$sql->servicename] = $sql->servicename;
}
$dropdown2 = array();
foreach($query2 as $sql){
	$dropdown2[$sql->petname] = $sql->petname;
}
?>
			...
			<table>
				<tr>
					...
					<td>
						...
						<?php
						if (isset($name) && isset($address) && isset($email) && isset($phone) && ($password) && ($type_of_service) && ($pet)) {
							echo validation_errors();
						}else {
							echo validation_errors();
							echo form_open();
							echo '<span style="color:#460da5;"><label for="name">*Name:</label><br/></span>' . form_input('name', set_value('name')) . '<br/><br/>';
							echo '<span style="color:#460da5;"><label for="address">*Address:</label><br/></span>' . form_input('address', set_value('address')) . '<br/><br/>';
							echo '<span style="color:#460da5;"><label for="email">*Email:</label><br/></span>' . form_input('email', set_value('email')) . '<br/><br/>';
							echo '<span style="color:#460da5;"><label for="phone">*Phone:</label><br/></span>' . form_input('phone', set_value('phone')) . '<br/><br/>';
							echo '<span style="color:#460da5;"><label for="password">*Password:</label><br/></span>' . form_password('password', set_value('password')) . '<br/><br/>';
							echo '<span style="color:#460da5;"><label for="type_of_service">*Type of Service:</label><br/></span>' . form_dropdown('select1', $dropdown1, 'Select Service') . '<br/><br/>';
							echo '<span style="color:#460da5;"><label for="Pet_Name">*Pet Name:</label><br/></span>' . form_dropdown('select', $dropdown2, 'Select Pet') . '<br/><br/>';
